...
const EditBankTransfer = () => {
    ...
    const { editBankTransfer, getBankTransfer, uploadQrCodeImage, loading } = useBankTransfer();
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getBankTransfer(id);
                setFormData({
                    accoountHolderName: data.accoountHolderName || '',
                ...
                if (data.qrCodeImage) {
                    setQrPreview(`${SERVER_URL}/${data.qrCodeImage}`);
                }
            } catch (error) {
    ...
    const handleQrUpload = useCallback(async (e) => {
        ...
        try {
            ...
            setQrPreview(formData.qrCodeImage ? `${SERVER_URL}/${formData.qrCodeImage}` : null);
        } finally {
            setUploading(false);
        }
    }, [uploadQrCodeImage, formData.qrCodeImage]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await editBankTransfer(id, formData);
            navigate('/admin');
        } catch (error) {
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6">
                    <h1 className="text-3xl font-bold text-white">Edit Bank Account</h1>
                    <p className="text-blue-100 mt-2">Update bank account details</p>
                </div>
